...
double external_potential_prime(double val) //derivata prima?
{
	return 4.0*pow(val,3) - 5.0*val;
}

double external_potential_second(double val)  //derivata seconda??
{
...
double variationalWaveFunction(double v)
{
	...
	double mu=0.8;
	double sigma = 0.6;
	double espo1 = (v-mu)*(v-mu)/2./sigma/sigma;
  	double espo2 = (v+mu)*(v+mu)/2./sigma/sigma;
	return exp(-espo1) + exp(-espo2);
}
...
double variationalWaveFunction_second(double v)
{
	...
	double espo1 = (v-mu)*(v-mu)/2./sigma/sigma;
  	double espo2 = (v+mu)*(v+mu)/2./sigma/sigma;
    double dev = (-1./pow(sigma,2) + (v-mu)*(v-mu)/pow(sigma,4))*exp(-espo1) 
      + ( -1./pow(sigma,2) + (v+mu)*(v+mu)/pow(sigma,4) ) *exp(-espo2);
	return dev;
}
...
{
	ofstream out( "potential_pimc3.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out( "kinetic_pimc3.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out( "probability_pimc3.dat");
